...
class MyTestCase(unittest.TestCase):
    ...
    def test_outcar(self):
        ...
        """
        vo = pychemia.code.vasp.VaspOutput('pychemia/test/data/vasp_06/OUTCAR')
    ...
    def test_poscar(self):
        ...
        with self.assertRaises(ValueError) as context:
            st = pychemia.code.vasp.read_poscar('pychemia/test/data/vasp_07/POSCAR')

        st = pychemia.code.vasp.read_poscar('pychemia/test/data/vasp_08/POSCAR_old')
        self.assertEqual(st.natom, 2)

        st = pychemia.code.vasp.read_poscar('pychemia/test/data/vasp_08/POSCAR_new')
        self.assertEqual(st.natom, 2)
        ...
        cwd = os.getcwd()
        os.chdir('pychemia/test/data/vasp_07')
        st = pychemia.code.vasp.read_poscar('POSCAR_new')
        os.chdir(cwd)
        self.assertEqual(st.natom, 44)

        st = pychemia.code.vasp.read_poscar('pychemia/test/data/vasp_07/POSCAR_alt')
